...
describe Oculus::Query do
  ...
  it "allows updating the query field" do
    connection = stub(:thread_id => 42)
    query = Oculus::Query.create(:query => 'SELECT * FROM users')
    connection.should_receive(:execute).with('SELECT * FROM users')
    query.execute(connection)
    Oculus::Query.find(query.id).query.should == 'SELECT * FROM users' 
    query.update('SELECT count(*) FROM users')
    query = Oculus::Query.find(query.id)
    query.query.should == "SELECT count(*) FROM users"
  end 
end
