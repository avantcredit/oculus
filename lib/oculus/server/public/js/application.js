$(function() {
  ...
  $('.form-actions .save').click(function() {
    var sql = $('#query-field').val();
    var query_id = $('#query-form').data("id");
    UpdateQuery(query_id, sql);    
    window.location.href = '/oculus/queries/' + query_id;
  });

  $('.form-actions .revert').click(function() {    
    var query_id = $('#query-form').data("id")
    window.location.href = '/oculus/queries/' + query_id
  });
...
function UpdateQuery(id, sql) {
  $.ajax({
      url: '/oculus/queries/' + id + '/update',
      data: { query: sql },
      type: 'POST',
      success: 200
    });
...
Query.prototype.cancel = function() {
  $.ajax({
    url: '/oculus/queries/' + this.id + '/cancel',
    type: 'POST'
  });
}
...
Query.prototype.check = function(callbacks) {
  $.ajax({
    url: '/oculus/queries/' + this.id + '/status',
    type: 'GET',
...
Query.prototype.load = function(callbacks) {
  ...
  $.ajax({
    url: '/oculus/queries/' + this.id + '.json',
    type: 'GET',
    dataType: 'json',
